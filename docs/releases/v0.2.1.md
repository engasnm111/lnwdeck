...
The updater uses the NSIS installers and `latest.json`. This release also carries
MSI signatures without the MSI bodies, because the build job did not upload
`*.msi`; that is fixed for the next release, and the MSI is not required to
