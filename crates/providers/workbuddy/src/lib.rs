...
//!
//! Claude-Code-fork session JSONL under `.workbuddy/projects` is scanned read-only with
...
impl WorkbuddyAdapter {
    pub fn new() -> Self {
        let home = std::env::var_os("USERPROFILE")
            ...
            .map(PathBuf::from)
            .unwrap_or_default();
        Self::with_roots(vec![home.join(".workbuddy/projects")])
    }
    ...
    fn health_check(&self) -> AdapterHealth {
        ...
        if !detection.source_exists {
            return AdapterHealth {
                ...
                message: "WorkBuddy local data not found".to_string(),
            };
        }
        if detection.detected {
            AdapterHealth {
                status: AdapterHealthStatus::Healthy,
                message: "WorkBuddy local records detected".to_string(),
            }
        } else {
            AdapterHealth {
                status: AdapterHealthStatus::Degraded,
                message: "WorkBuddy local data has no token records".to_string(),
            }
        }
    }
...
mod tests {
    ...
    fn write_session(root: &std::path::Path, name: &str) {
        ...
        std::fs::write(
            dir.join(name),
            r#"{"type":"assistant","timestamp":"2027-01-15T10:00:00Z","message":{"usage":{"input_tokens":100,"output_tokens":50},"model":"tencent-hy3"}}"#,
        )
    ...
    #[test]
    fn id_is_correct() {
        assert_eq!(WorkbuddyAdapter::new().id(), "workbuddy");
    }
    ...
    fn reads_local_sessions_and_reports_usage() {
        ...
        let adapter = WorkbuddyAdapter::with_roots(vec![root]);
        ...
        assert!(
            ...
            "local WorkBuddy records are not a published quota"
        );
    ...
    fn missing_root_reports_no_data() {
        let dir = tempfile::tempdir().expect("temp dir");
        let adapter = WorkbuddyAdapter::with_roots(vec![dir.path().join("missing")]);
        assert_eq!(
