...
//!
//! Claude-Code-fork session JSONL under `.codebuddy/projects` is scanned read-only with
...
impl CodebuddyAdapter {
    pub fn new() -> Self {
        let home = std::env::var_os("USERPROFILE")
            ...
            .map(PathBuf::from)
            .unwrap_or_default();
        Self::with_roots(vec![home.join(".codebuddy/projects")])
    }
    ...
    fn health_check(&self) -> AdapterHealth {
        ...
        if !detection.source_exists {
            return AdapterHealth {
                ...
                message: "CodeBuddy local data not found".to_string(),
            };
        }
        if detection.detected {
            AdapterHealth {
                status: AdapterHealthStatus::Healthy,
                message: "CodeBuddy local records detected".to_string(),
            }
        } else {
            AdapterHealth {
                status: AdapterHealthStatus::Degraded,
                message: "CodeBuddy local data has no token records".to_string(),
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn id_is_correct() {
        assert_eq!(CodebuddyAdapter::new().id(), "codebuddy");
    }
    ...
    fn reads_local_sessions_and_reports_usage() {
        ...
        let adapter = CodebuddyAdapter::with_roots(vec![root]);
        ...
        assert!(
            ...
            "local usage is not a published CodeBuddy quota"
        );
    ...
    fn missing_root_reports_no_data() {
        let dir = tempfile::tempdir().expect("temp dir");
        let adapter = CodebuddyAdapter::with_roots(vec![dir.path().join("missing")]);
        assert_eq!(
